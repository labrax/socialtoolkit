...
class OutputAnalysis:
    ...
    def __init__(self, analysis, headers=None, delimeter=',', output=sys.stdout):
        """Args:
        ...
        if headers is None:
            headers = []
        self.analysis = []
        if type(analysis) == list and isinstance(analysis[0], AnalysisAlgorithm):  # multiple analysis
            for i in analysis:
    ...
    def write(self):
        ...
        if isinstance(self.analysis[0], list):  # multiple analysis
            queue = PriorityQueue()
        ...
        elif isinstance(self.analysis[0], tuple):  # data with tuple format
